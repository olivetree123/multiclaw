...
PRODUCT_MANAGER_SPEC = AgentSpec(
    ...
    role_prompt=("你是一名产品经理。\n"
                 ...
                 "- 需要需求澄清时，回复开头加上：<!-- STATUS: needs_clarification -->\n"
                 "- 需求完整但用户未确认，回复开头加上：<!-- STATUS: needs_confirmation -->\n"
...
FRONTEND_PROTOTYPE_ROLE_PROMPT = ("你是一名前端工程师，当前处于【样品开发】阶段。\n"
...
FRONTEND_INTEGRATION_ROLE_PROMPT = ("你是一名前端工程师，当前处于【接口对接】阶段。\n"
...
FRONTEND_MAINTENANCE_ROLE_PROMPT = ("你是一名前端工程师，当前处于【维护】模式。\n"
                                    "- 应用已构建并完成 API 对接。按每次任务做针对性修改。\n"
                                    "- 以 backend/docs/openapi.yaml 为 API 权威来源。\n"
                                    "- 除非任务明确要求变更，否则保持现有行为。\n"
                                    "- 后端 API 变更后，确保前端保持兼容。")

MAINTENANCE_COORDINATOR_ROLE_PROMPT = ("你是项目协调员，当前处于【维护】模式。\n"
...
def load_project_context(project_root: Path) -> dict[str, str | None]:
    ...
    if openapi_path.is_file():
        context["backend_result"] = (f"API 契约：{openapi_path}\n\n"
